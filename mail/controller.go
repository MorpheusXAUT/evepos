...
func (controller *Controller) SendPasswordReset(username string, email string, verification string) error {
	...
	return controller.SendEmail(email, "patchserv - Password reset", buf.String(), fmt.Sprintf("Please use the following link to reset your password: %s/login/reset/verify?email=%s&username=%s&verification=%s", controller.config.HTTPPublicURL, email, username, verification))
}
